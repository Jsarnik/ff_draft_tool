...
class Navigation extends Component {
    ...
    componentDidUpdate = (prevProps) => {
      if(this.props.location.pathname && this.props.location.pathname !== prevProps.location.pathname){
        ...
        document.title = `ESPN Fantasy Tools - ${_split[_split.length-1] || 'login'}`;
      }
    }
    ...
    render() {
      return(
        <Header style={{ position: 'fixed', zIndex: 100, width: '100%', height: '30px' }}>
          <div className="logo" style={{height: '30px', fontSize: '120%', margin: 0, background: 'none', width: '300px', float: 'left', 'lineHeight': '30px', color: 'orange'}}>ESPN Fantasy Tool</div>
            <Menu
              theme="dark"
              mode="horizontal"
